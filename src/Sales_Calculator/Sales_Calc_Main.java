...
public class Sales_Calc_Main
{
    ...
    { 
        ...
        //Start of order taking loop. Allows user to keep adding items to their 
        //order. After an item is added, the confirm asks if they want to add 
        //another item Sentinel value anything but "No" will start the loop over 
        //and allow for new item to be added to order. Starts with 1st question 
        while (!"no".equals(answer = JOptionPane.showInputDialog
                ("Would you like to add something to your cart?" +
                "\nType no if you don't want to add anything to your cart" +
                "\nIf you enter anything else but no I will assume you want to "
                        + "add something to your cart")))
        {   
            //asks the 2nd question & validates is not empty
            while (getUserName1.isEmpty())
            {
                getUserName1 = JOptionPane.showInputDialog(
                        "Please enter your name");
            }
            
            productIdNew1= ""; 
            
            //asks the 3rd question & validates is not empty
            ...
            //validates 3rd questions input is within range. Add try/catch
            ...
            //asks 4th question and validates is not empty
            while (productNameNew.isEmpty())
            {
                productNameNew = JOptionPane.showInputDialog(
                        "Enter the product name");
            }
            
            //asks 3rd question. Add try/catch 
        ...
        //Shows order confirmation message with eight variables and provides 
        //y/n to place order. Can see the obect count, item count, average sales, 
        //shipping cost, and total including shipping and item totals    
        JOptionPane.showConfirmDialog(null,"To confirm, " + 
                getUserName2.userNameGet()+
                " the " + objectcount + " object I mean item (I keep forgetting "
                + "to speak human) you placed in your cart was:\n\nProduct ID: " 
                + computeAverageSales[objectcount].productIdGet() +
                "\nProduct Name: " + 
                computeAverageSales[objectcount].productNameGet() + 
                "\nProduct product Price: $" + 
                computeAverageSales[objectcount].productPriceGet() + 
                "\n\nSo adding everything in your cart up. You have: " +
                "\nA total of items of: " + 
                computeAverageSales[objectcount].itemCountGet() + 
                "\nThe average cost of items in your cart is: $" + 
                (computeAverageSales[objectcount].totalPriceGet()/ 
                computeAverageSales[objectcount].itemCountGet() )+
                "\nTotal shipping cost: $" + 
                computeShippingCharges1.shippingCostGet()+
